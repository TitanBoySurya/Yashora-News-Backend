...
export const processContent = async (text: string, lang: string) => {
  try {
    const res = await axios.post(
      ...
      {
        model: "gpt-5o-mini", // Sasta aur fast model, perfect hai
        messages: [
          {
            role: "system",
            content: `You are a professional news editor. 
            Tasks:
            1. Summarize the news in exactly 80-1000 words.
        ...
        max_tokens: 3000, // Taaki summary puri aaye aur beech mein na kate
        temperature: 0.7,
      },
      {
        headers: {
    ...
    return text.length > 1600 ? text.slice(0, 1570) + "..." : text;
  }
};